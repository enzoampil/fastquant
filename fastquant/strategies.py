    ...
    def next(self):
        ...
        if self.cash >= self.dataclose[0]:
            if self.rsi[0] < self.rsi_lower:
                ...
                buy_prop_size = int((self.cash / self.dataclose[0]) * self.buy_prop)
    ...
    def next(self):
        ...
        if self.cash >= self.dataclose[0]:
            if self.buy_signal():
                ...
                afforded_size = int(
                    self.cash
                    / (self.dataclose[0] * (1 + COMMISSION_PER_TRANSACTION * 3))
                )
                buy_prop_size = int(afforded_size * self.buy_prop)
                # Buy based on the closing price of the next closing day
                if self.execution_type == "close":
            ...
            if self.sell_signal():
                ...
                if self.execution_type == "close":
                    if SELL_PROP == 1:
                        ...
                        self.order = self.sell(
                            size=int(
                                (stock_value / (self.dataclose[0])) * self.sell_prop
                            ),
                            exectype=bt.Order.Close,
                        )
                else:
                    # Sell based on the opening price of the next closing day (only works "open" data exists in the dataset)
                    self.order = self.sell(
                        size=int((self.init_cash / self.dataopen[0]) * self.sell_prop),
                    )

class RSIStrategy2(BaseStrategy):
    params = (
        ("rsi_period", 14),
        ("rsi_upper", 70),
        ("rsi_lower", 30),
...
STRATEGY_MAPPING = {"rsi": RSIStrategy, "smac": SMACStrategy, "base": BaseStrategy, "rsi2": RSIStrategy2}
