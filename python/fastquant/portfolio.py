...
class Portfolio:
    ...
    def calculate_portfolio_risk(self, weights):
        ...
        portfolio_risk = np.sqrt(np.dot(weights.T, np.dot(self.returns.cov()*252, weights)))
        if self.verbose:
    ...
    def optimize_portfolio(self):
        ...
        init_weights = self.random_weights if self.optimum_weights is None else self.optimum_weights
        optimum = optimization.minimize(fun=self.min_func_sharpe,
                                      x0=init_weights,
                                      method='SLSQP',
                                      bounds=bounds,
    ...
    def print_optimal_portfolio(self):
        ...
        print("Expected return, volatility and Sharpe ratio:", self.calculate_statistics(self.optimum_weights))
